<script lang="ts">
    ...
    import { ListImages,
        ...
        ListContainers,
        GetContainerLogs,
        OpenDialogError } from '../../wailsjs/go/backend/Backend';
...
async function listImages() {
    try {
        ...
        images.set(result.map(imageString => {
            const [repository, imageId, tag, created, size] = imageString.split(/\s+/);
            const createdDate = new Date(parseInt(created) * 1000).toLocaleString();
            const sizeMB = (parseInt(size, 10) / 1024 / 1024).toFixed(2) + 'MB';
            return { repository, tag, imageId, created: createdDate, size: sizeMB };
        }));
    } catch (e) {
...
async function listContainers() {
    try {
        ...
        containers.set(result.map(containerString => {
            const [containerId, image, name , command, ports, status, created] = containerString.split(/\s+/);
            const createdDate = new Date(parseInt(created) * 1000).toLocaleString();
            return { containerId, image, name, command, ports, status, created: createdDate };
        }));
    } catch (e) {
...
async function getContainerLogs(containerId: string) {
    try {
        ...
        OpenDialogError(`Failed to get container logs: ${containerId} - ${e.message}`);
    }
}
        ...
        <tbody>
            {#each $images as image}
                <tr>
                    ...
                    <td>{image.created}</td>
                    <td>{image.size}</td>
                    <td><button on:click={() => removeImage(image.imageId)}>Remove</button></td>
                </tr>
            {/each}
        </tbody>
    </table>
        ...
        <tbody>
            {#each $containers as container}
                <tr>
                    ...
                    <td><button on:click={() => getContainerLogs(container.containerId)}>Logs</button></td>
                </tr>
            {/each}
        </tbody>
    </table>
